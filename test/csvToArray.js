...
import csvToArray from "../challenges/csvToArray.js";

describe("csvToArray.js", () => {
  it(
    "should return the correct array of objects with a csv with no spaces",
    mochaAsync(async () => {
      try {
  ...
  it(
    "should return the correct array of objects with a csv with spaces",
    mochaAsync(async () => {
      try {
        const result = csvToArray(
          "Company Name,Ticker Symbol,Price Paid,Quantity\nGENERAL ELECTRIC,GE,12,100\nVisa,V,100,10\nJP Morgan,JPM,13,123"
        );
        result.should.be.a("array");
        result.length.should.equal(3);
